...
def _get_variables_value(self, record):
    value_by_name = {}
    for variable in self:
        value = variable._find_value_from_field_chain(record) if variable.field_type == 'field' else variable.demo_value
...
def _find_value_from_field_chain(self, record):
    ...
    try:
        field_value = reduce(lambda rec, fld: rec[fld], self.field_name.split('.'), record)
    except KeyError:
